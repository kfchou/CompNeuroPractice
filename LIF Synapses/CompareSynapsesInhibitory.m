...
s.cons(end).mechanism_list='iampa';
s.cons(end).parameters={'delay',0,'gSYN',gsyn,'netcon',BCnetcon,'ESYN',-80};
...
for i = 1:nPops
    ...
    subplot(5,1,2);
    plot(data.time, data.(['B_A_' (synapses{1}) '_Isyn'])); title(['synaptic current:' synapses{i}])
    subplot(5,1,3);
    plot(data.time, data.B_V); title('B voltage')
    subplot(5,1,4);
    plot(data.time, data.(['C_B_' (synapses{1}) '_Isyn'])); title(['synaptic current:' synapses{i}])
    subplot(5,1,5);
    plot(data.time, data.C_V); title('C voltage')
end
